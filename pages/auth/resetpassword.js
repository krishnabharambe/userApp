...
export default function Home() {
  ...
  const HandleRegisterSubmit = async (event) => {
    ...
    if (password == cpassword) {
      const res = await axios.post(
        'https://krishnabharambe.pythonanywhere.com/api/ForgetPasswordChange/',
  ...
  const HandleOtpSubmit = async (event) => {
    ...
    if (otp) {
      const res = await axios.post(
        'https://krishnabharambe.pythonanywhere.com/api/ForgotValidateOTP/',
  ...
  const HandleValidatePhone = async (event) => {
    ...
    if (phone) {
      ...
      if (phone.match(phonenoRegEx)) {
        ...
        setphone(phone);
        const res = await axios.post(
          'https://krishnabharambe.pythonanywhere.com/api/ValidatePhoneForgot/',
          { phone }
        );
        if (res.data.status) {
          //success
